...
- **Fechamento/Abertura de Caixa** (caixa_sessoes): overlay bloqueante (tela cheia) que lembra o funcionário de abrir (8h) e fechar (17h seg–sex, 13h sábado; domingo SEM bloqueio) o caixa. Abrir registra o valor inicial (troco); fechar calcula totalEntradas/totalSaídas do dia e o valorFinal (= inicial + entradas − saídas). Ao fechar (overlay E botão manual) há um campo editável "Quanto tem na gaveta agora?" pré-preenchido com o esperado; o valor digitado é salvo em `valor_contado` (coluna nullable, ADD COLUMN IF NOT EXISTS via ensureSchema) e o overlay mostra a diferença ("Sobrando/Faltando R$ X"). `valorFinal` continua sendo o esperado calculado; `valorContado` é o conferido real. 1 sessão por dia. No topo do Caixa modal há um resumo "Caixa de hoje" (status aberto/fechado, hora que abriu/fechou, troco inicial, entrou hoje, saiu hoje, "em caixa agora") via GET /caixa-sessoes (sessão do dia em SP); abaixo, histórico detalhado (data, abriu/fechou, valores) numa seção retrátil. **Fuso horário é AUTORIDADE DO BACKEND**: o servidor deriva "hoje" em America/Sao_Paulo (`hojeSP()`), nunca confia na `data` enviada pelo cliente para abrir/fechar/status; o overlay calcula a hora atual SP via `toLocaleString("en-US",{timeZone:"America/Sao_Paulo"})` (ignora o fuso do aparelho). Horas de abertura/fechamento exibidas com `toLocaleTimeString({timeZone:"America/Sao_Paulo"})`. Tabela criada na prod via `ensureSchema` CREATE TABLE IF NOT EXISTS. Usa raw fetch + react-query (não toca em api.ts gerado).
- **Importar nota do fornecedor** (AI): botão "Importar" ao lado de "Nova" na aba Peças. Usuário envia foto/PDF da nota do fornecedor → frontend converte em base64 e POST `/api/pecas/importar-nota` → backend manda pra IA (Gemini `gemini-2.5-flash`, responseMimeType JSON, via `@workspace/integrations-gemini-ai` proxy Replit, sem API key do usuário) que extrai `{modelo, quantidade, custo, qualidade}` de cada item (NÃO grava nada). Abre um diálogo de prévia (`ImportarNotaDialog`) com 1 linha editável por modelo: modelo/qualidade(Select QUALIDADES)/quantidade/custo/preço cliente/preço lojista, com preços auto-sugeridos pelo custo (`sugestaoPrecoCliente` = custo·2+mão-de-obra; `sugestaoPrecoLojista` = custo·1,7, ambos arredondados a R$5). "Cadastrar todos" → POST `/api/pecas/importar/confirmar` que insere cada item como TWIN (cliente+lojista) numa ÚNICA transação (tudo ou nada). Guards: MIME só image/*|application/pdf, tamanho ~7MB (limite inline Gemini ~8MB; `express.json` limit bumped p/ 25mb), quantidade ≥ 1. Usa raw fetch + react-query (não toca api.ts gerado). **Sem auth server-side** (igual todo o resto da API — login é só no frontend).
...
- `POST /api/pecas/importar-nota` — send `{fileBase64, mimeType}` (foto/PDF da nota); returns `{itens:[{modelo,quantidade,custo,qualidade}]}` extraídos via Gemini (não grava nada)
- `POST /api/pecas/importar/confirmar` — send `{itens:[{modelo,qualidade,quantidade,valorCusto,valorCliente,valorLojista}]}`; insere todos como twins (cliente+lojista) numa transação; returns `{cadastrados}`
