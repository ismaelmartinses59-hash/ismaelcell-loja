...
- **A Receber — compartilhar no WhatsApp como IMAGEM**: botão por conta gera uma imagem "EXTRATO DE DÉBITO" (marca ISMAEL CELL) via html2canvas e abre o compartilhamento nativo (preview → `navigator.share({files})` → download fallback). A seção "Produtos e Serviços" é DINÂMICA: 1 linha por item (`itens.map` ordenado oldest-first, sem linhas placeholder), card cresce de altura sozinho. Total = `saldo` devedor. Reaproveita a mesma infra de share das peças (`sharePreview`/`generatingShare`/`confirmShare`); card oculto em `extratoShareRef` gated em `sharingConta`. Número do rodapé fixo conforme template (89 98144-8787).
  - O markup do extrato vive no componente reutilizável `ExtratoCard` (`src/components/extrato-card.tsx`, `forwardRef`). **IMPORTANTE p/ html2canvas 1.4.1**: o card usa SÓ posicionamento absoluto (ícones em `position:absolute` + texto em bloco com `left/top/width`), porque flex / table-cell / inline-block+vertical-align renderizam TORTO na imagem. Nunca reintroduzir flex/table nas regiões capturadas.
