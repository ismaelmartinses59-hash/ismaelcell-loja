          ...
          {tab === "registrar" && (
            <div className="space-y-3">
              ...
              <div className="rounded-xl border bg-muted/30 overflow-hidden">
                <button
                  type="button"
                  onClick={() => setShowOSBusca((v) => !v)}
                  className="w-full flex items-center justify-between px-3 py-2.5 text-xs font-semibold text-muted-foreground hover:bg-muted/60 transition-colors"
                >
                  <span className="flex items-center gap-1.5">
                    <Search className="w-3.5 h-3.5" />
                    Preencher por OS (opcional)
                  </span>
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform ${showOSBusca ? "rotate-180" : ""}`} />
                </button>
                {showOSBusca && (
                  <div className="px-3 pb-3 space-y-2 border-t">
                    <div className="flex gap-2 pt-2">
                      <Input
                        placeholder="Ex: OS-1234567890"
                        value={busca}
                    ...
                    {loadingRegistrar && buscaAtiva && (
                      <div className="flex items-center gap-2 text-xs text-muted-foreground py-1 justify-center">
                        <Loader2 className="w-3.5 h-3.5 animate-spin" /> Buscando...
                      </div>
                    )}
                    {buscaAtiva && !loadingRegistrar && !orderRegistrar && (
                      <p className="text-xs text-center text-muted-foreground py-1">
                        Nenhuma OS encontrada para <b>{buscaAtiva}</b>
                      </p>
                    )}
                    {orderRegistrar && (
                      <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-3 py-2">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-green-800 truncate">{orderRegistrar.modelo}</p>
                          <p className="text-[10px] text-green-600 font-mono">#{orderRegistrar.codigo}</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => { setBusca(""); setBuscaAtiva(""); setNomeRegistrar(""); setDataRegistrar(format(new Date(), "yyyy-MM-dd")); setGarantiaSelecionada(""); }}
                          className="ml-auto text-green-400 hover:text-green-700 shrink-0"
                        >
              ...
              <div className="rounded-xl border bg-muted/40 p-4 space-y-3">
                ...
                {orderRegistrar ? (
                  <Button
                    className="w-full"
                    onClick={handleSalvarRegistrar}
                    disabled={!garantiaSelecionada || editOrder.isPending}
                  >
                    {editOrder.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-2" />}
                    Salvar Garantia
                  </Button>
                ) : (
                  <Button
                ...
                {(garantiaSelecionada && orderRegistrar) || ordemCriada ? (
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t">
                    {(() => {
                      const ordemParaDoc = ordemCriada ?? orderRegistrarEfetivo!;
                      return (
                        <>
                          <Button
                            variant="outline"
                            className="w-full"
                            onClick={() => handleEnviarWhats(ordemParaDoc)}
                            disabled={enviandoId === ordemParaDoc.id}
                          >
                            {enviandoId === ordemParaDoc.id
                              ? <Loader2 className="w-4 h-4 mr-1 animate-spin" />
                              : <Share2 className="w-4 h-4 mr-1" />}
                            WhatsApp
                          </Button>
                          <Button variant="outline" className="w-full" onClick={() => printGarantia(ordemParaDoc)}>
